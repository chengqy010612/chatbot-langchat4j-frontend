...
import { chatListService } from "../api";
...
const Independent: React.FC = () => {
  ...
  const initChatList = async () => {
    const { data } = await chatListService();
    const conversationsList = data.map((item) => {
      return {
        ...item,
        key: item.memoryId,
        label: "What is Ant Design X?",
  ...
  //         onUpdate({ role: "assistant", content: currentContent });
  //         if (currentContent === fullContent) {
  //           clearInterval(id);
  //           //   onSuccess([fullContent]);
  //           onSuccess([{ role: "assistant", content: fullContent }]);
  //         }
  //       }, 100);
  //     },
  //   });
  ...
  const [agent] = useXAgent<string, { message: string }, string>({
    request: async ({ message }, { onSuccess, onUpdate }) => {
      ...
      await axios
        .post(
          "/api/xiaozhi/chat",
          { memoryId: curConversation, message: message.content },
          //   { memoryId: 7, message: message.content },
          {
            ...
            onDownloadProgress: (e) => {
              ...
              fullContent = e.event.target.responseText; // 累积的完整文本
              let currentContent = "";
              currentContent = fullContent.substring(lastMsg.length);
              lastMsg += currentContent;
              onUpdate({ role: "assistant", content: fullContent });
            },
          }
        )
        .then((res) => {
